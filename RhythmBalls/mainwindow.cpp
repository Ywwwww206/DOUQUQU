    ...
    : QMainWindow(parent)
{
    ...
    m_mapRect->setPos(20, 75);          // 垂直居中（之前是50）
...
void MainWindow::updatePhysics()
{
    ...
    const qreal subDt = 1.0 / subSteps;   // 用于调整速度或加速度的比例（此处无需修改advance，因为advance内部使用固定重力）

    for (int step = 0; step < subSteps; ++step) {
        // 1. 所有球自由运动
        m_scene->advance();

        // 2. 碰撞处理（在子步骤结束时立即解决，避免累积穿透）
        handleCollisions();
    }

    // 3. 清理死亡小球（只在一帧结束时清理就行）
...
void MainWindow::handleCollisions()
{
    ...
    for (int iteration = 0; iteration < 2; ++iteration) {
        ...
        if (!anyCollision) break;   // 如果没有碰撞，提前退出迭代
    }
}
